...
**玻尔兹曼机和霍普菲尔网络很接近，差别只是：一些神经元作为输入神经元，剩余的则是作为隐神经元。**  
...
**稀疏自编码机（SAE：Sparse autoencoders）**  
**稀疏自编码机某种程度上同自编码机相反。**稀疏自编码机不是用更小的空间表征大量信息，而是把原本的信息编码到更大的空间内。因此，中间层不是收敛，而是扩张，然后再还原到输入大小。它可以用于提取数据集内的小特征。  
...
**变分自编码机（VAE：Variational autoencoders）**  
**变分自编码机和AE有着相同的架构，却被教会了不同的事情：输入样本的一个近似概率分布，这让它跟BM、RBM更相近。**  
...
**去噪自编码机（DAE：Denoising autoencoders）**  
**去噪自编码机是一种自编码机，它的训练过程，不仅要输入数据，还有再加上噪音数据（就好像让图像变得更加模糊一样）。**  
...
**池化**是一种过滤掉细节的方式：一种常用的池化方式是最大池化，比如用2X2的像素，然后取四个像素中值最大的那个传递。为了让卷积神经网络处理语音数据，需要把语音数据切分，一段一段输入。在实际应用中，通常会在卷积神经网络后面加一个前馈神经网络，以进一步处理数据，从而对数据进行更高水平的非线性抽象。

**解卷积网络（DN：Deconvolutional networks）**  
...
**深度卷积逆向图网络（DCIGN：Deep convolutional inverse graphics networks）**  
**这个名字具有误导性，因为它们实际上是VAE，但分别用CNN、DNN来作编码和解码的部分**  
...
具有时间联结的前馈神经网络：它们有了状态，通道与通道之间有了时间上的联系。神经元的输入信息，不仅包括前一神经细胞层的输出，还包括它自身在先前通道的状态。  
...
**试图通过引入门结构与明确定义的记忆单元来解决梯度消失/爆炸的问题。**  
...
**门循环单元（GRU : Gated recurrent units）**  
...
**神经图灵机（NTM: Neural Turing machines）**  
可以理解为对LSTM的抽象，它试图把神经网络去黑箱化（以窥探其内部发生的细节）。  
...
**深度残差网络（DRN: Deep residual networks）**  
非常深的FFNN网络，它有一种特殊的连接，可以把信息从某一神经细胞层传至后面几层（通常是2到5层）。  
...
**回声状态网络（ESN：Echo state networks）**  
另一种不同类型的循环网络  
...
**Kohonen网络**  
KN，也称之为自组织（特征）映射（SOM/SOFM：self organising (feature) map）  
...
资料来源：https://mp.weixin.qq.com/s/Si2jtA3jrKJVCXJ1B3fZQw  
